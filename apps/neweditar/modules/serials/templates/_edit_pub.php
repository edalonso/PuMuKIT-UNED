...
<!-- SELECT -->
<select name="status" id="filters_anounce" onchange="$('#pub_mm_info').load('<?php echo url_for('mms/update_pub?id=' . $mm->getId())?>/status/' + this.value)">
...
<div class="form-row">
  ...
  <div class="content content_long">
     <input type="checkbox" name="youtube" value="1"  id="youtube" <?php //echo ($mm->getMmYoutube() === null)? '':'checked="checked"' ?> onchange="$('#youtube_mm_info').load('<?php echo url_for('mms/update_yt?id=' . $mm->getId())?>/youtube/' + this.checked + '/list/'+ document.getElementById('YtLists').value)"> 
...
<div class="content content_long">
    ...
    <select name="YtList" id="YtLists" onchange=" if (document.getElementById('youtube').checked) {
        $('#youtube_mm_info').load('<?php echo url_for('mms/update_yt?id=' . $mm->getId())?>/youtube/' + document.getElementById('youtube').checked + '/list/'+ this.value)}">
...
<div class="form-row">
  ...
  <div class="content content_long">
    <?php if(count($mm->getSerial()->getSerialItuness()) == 0):?>
      <a href="#" onclick="$('#itunes_mm_info').load('<?php echo url_for('mms/ituneson?id=' . $mm->getId())?>'); return false;">Publicar en itunes U.</a>
    <?php else:?>
      <a href="#" onclick="$('#itunes_mm_info').load('<?php echo url_for('mms/ituneson?id=' . $mm->getId())?>'); return false;
