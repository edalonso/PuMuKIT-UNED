...
<table cellspacing="0" class="tv_admin_list">
  ...
  <?php $t = count($mms) ; for( $i=0; $i<$t; $i++): $mm = $mms[$i]; $odd = fmod($i, 2) ?>
    <tr onmouseover="$(this).addClass('tv_admin_row_over');"  onmouseout="$(this).removeClass('tv_admin_row_over');" id="complet_tr_<?php echo $mm->getId()?>" class="drop tv_admin_row_<?php echo $odd ?><?php if($i == 0) echo ' tv_admin_row_this'?>" >
      <td>
...
<a href="#" onclick="edit_mms(<?php echo $mm->getId()?>); return false;"><img alt="editar" title="editar" class="miniTag" src="/images/admin/mbuttons/edit_inline.gif"></a>
      ...
      <td>
        <?php foreach($mm->getCategorys($cat_raiz_unesco) as $unesco): ?>
         <span title="<?php echo $unesco->getName()?>" style="font-size: 8px; padding: 1px 5px;margin:2px;<?php if ($unesco->getName()=='INFORMATIVOS Y CULTURALES') echo 'background-color:blue'?><?php if ($unesco->getName()=='Historia') echo 'background-color:green'?>" class="label label-success circulo"></span>
        <?php endforeach ?>
        <?php foreach($mm->getCategorys($cat_raiz_uned) as $uned): ?>
         <span title="<?php echo $uned->getName()?>" style="font-size: 8px; padding: 1px 5px;margin:2px;<?php if ($uned->getName()=='INFORMATIVOS Y CULTURALES') echo 'background-color:blue'?><?php if ($uned->getName()=='Historia') echo 'background-color:green'?>" class="label label-info circulo"></span>
        <?php endforeach ?>
      </td>
    </tr>
  <?php endfor; ?>
