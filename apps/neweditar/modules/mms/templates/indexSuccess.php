...
<div id="tv_admin_container">
  ...
  <div id="tv_admin_content" >
    ...
    <select id="options_mms" style="margin: 10px 0px; width: 33%" title="Acciones sobre elementos selecionados" onchange="window.change_select('mm', $('options_mms'))">
      ...
      <option value="inv_announce_sel">Marcar/Desmarcar como novedad selecionados</option>
      <option value="inv_important_sel">Marcar/Desmarcar como autónomos selecionados</option>
      <option disabled="">---</option>
      ...
      <option disabled="">---</option>
      <option value="cut_mm">Cortar objetos multimedia seleccionados</option> 
      <?php if(count($sf_user->getAttribute('cut_mms', array())->getRawValue()) != 0): ?>
        <option value="paste_mm">
          Peger objetos multimedia (<?php echo implode(',', $sf_user->getAttribute('cut_mms')->getRawValue())?>)
        </option>
      <?php endif ?>
      <option disabled="">---</option>
