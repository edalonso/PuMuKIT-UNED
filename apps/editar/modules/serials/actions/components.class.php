...
class serialsComponents extends sfComponents
{
  ...
  public function executeList()
  {
    ...
    //var_dump($this->total_serial_all);var_dump($this->total_serial);exit;
  ...
  protected function processFilters(Criteria $c)
  {
    ...
    if (isset($filters['status']) && $filters['status'] !== 'all'){
      $c->addJoin(SerialPeer::ID, MmPeer::SERIAL_ID);
      $c->add(SerialPeer::DISPLAY, $filters['status']=='false');
    }
    ...
    if (isset($filters['date'])){
      if (isset($filters['date']['from']) && $filters['date']['from'] !== ''){
        ...
        $criterion = $c->getNewCriterion(SerialPeer::PUBLICDATE, "$y-$m-$d", Criteria::GREATER_EQUAL);
      }

      if (isset($filters['date']['to']) && $filters['date']['to'] !== ''){
        if (isset($criterion)){
          list($d, $m, $y) = sfI18N::getDateForCulture($filters['date']['to'], $this->getUser()->getCulture());
          $criterion->addAnd($c->getNewCriterion(SerialPeer::PUBLICDATE, "$y-$m-$d", Criteria::LESS_EQUAL));
        }else{
          list($d, $m, $y) = sfI18N::getDateForCulture($filters['date']['to'], $this->getUser()->getCulture());
          $criterion = $c->getNewCriterion(SerialPeer::PUBLICDATE, "$y-$m-$d", Criteria::LESS_EQUAL);
        }
      }
