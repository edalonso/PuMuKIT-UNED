<table cellspacing="0" class="tv_admin_list">
  ...
  <?php $i = 1; foreach ($serials as $serial): $odd = fmod(++$i, 2); $numV = $serial['mm_count'] ?>
    <tr onmouseover="Element.addClassName(this,'tv_admin_row_over')" onmouseout="Element.removeClassName(this,'tv_admin_row_over')" class="tv_admin_row_<?php echo $odd ?> <?php if($serial['id'] == $sf_user->getAttribute('id', null, 'tv_admin/serial')) echo ' tv_admin_row_this'?>" >
      ...
      <td style="width:12px">
          <?php echo image_tag('admin/bbuttons/mm'.$serial['display'].$serial['mm_status_max'].'_inline.gif', array('alt' => 'estado del contenido', 'title'=>'Estado del contenido', 'id'=>'table_serials_display_' . $serial['id'])) ?>
      </td>
      <td  style="width:12px">
...
<?php if (isset($reloadPreviewAndList)): ?>
  <?php echo javascript_tag("
    new Ajax.Updater('preview_serial', '" . url_for('serials/preview?id=' . $sf_user->getAttribute('id', 'null', 'tv_admin/serial')) . "')
    new Ajax.Updater('edit_serials', '" . url_for('serials/edit?id=' . $sf_user->getAttribute('id', 'null', 'tv_admin/serial')) . "')
  ");?>
<?php endif;?>
