...
new Ajax.Updater('pub_mm_info', '<?php echo url_for('serials/updatePub')?>', {
        asynchronous:true, 
        evalScripts:true,
        parameters: 'status=' + $('status_select').value + '&ids=' + $$('.change_pub_mms:checked').invoke('getAttribute', 'id').toJSON()
}); return false">
...
<div class="form-row">
  ...
  <div class="content content_long">
    <?php if(count($serial->getSerialItuness()) == 0):?>
      <a href="#" onclick="
  new Ajax.Updater('itunes_mm_info', '<?php echo url_for('mms/ituneson?id=' . $mm->getId())?>', {
    asynchronous: true, 
    evalScripts: true,
...
">Publicar en itunes U.</a>
    <?php else:?>
      <a href="#" onclick="
  new Ajax.Updater('itunes_mm_info', '<?php echo url_for('mms/itunesoff?id=' . $mm->getId())?>', {asynchronous:true, evalScripts:true}); return false;
...
<?php endif ?>
    ...
    <ul class="tv_admin_actions">
      <li><?php echo button_to_function('Cerrar', "new Ajax.Updater('list_serials', '".url_for('serials/list')."', {asynchronous:true, evalScripts:true}); Modalbox.hide()", 'class=tv_admin_action_delete') ?> </li>
    </ul> 
