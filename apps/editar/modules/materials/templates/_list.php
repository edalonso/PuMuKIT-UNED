  ...
  <?php $t = count($materials) ; for( $i=0; $i<$t; $i++): $material = $materials[$i] //idea primer y ultimo apate ?>  
    <tr>
      ...
      <td><?php echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=borrar title=borrar'), array(
														    'update' => 'materials_mms', 'url' => 'materials/delete?id='.$material->getId().'&mm='.$mm, 'script' => 'true', 'confirm' => '&iquest;Seguro?',
														    'complete' => 'update_preview(' . $mm . ')'
														    ))?></td>
      ...
      <td><?php echo (( $i == 0) ? '&nbsp;' : (link_to_remote('&#8593;', array(
									       'update' => 'materials_mms', 'url' => 'materials/up?id='.$material->getId().'&mm='.$mm, 'script' => 'true',
									       'complete' => 'update_preview(' . $mm . ')'
									       ))))   ?></td>
      <td><?php echo (( $i == $t-1)? '&nbsp;' : (link_to_remote('&#8595;', array(
										 'update' => 'materials_mms', 'url' => 'materials/down?id='.$material->getId().'&mm='.$mm, 'script' => 'true',
										 'complete' => 'update_preview(' . $mm . ')'
