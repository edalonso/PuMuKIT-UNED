<table><tdody>
    ...
    <?php foreach($transcodings as $transcoding):?>
      <tr>
        ...
        <td colspan="9">
          ...
          <?php if($transcoding->getStatusId() == TranscodingPeer::STATUS_ERROR) echo link_to_remote(image_tag('admin/mbuttons/use_inline.gif', 'alt=retranscodificar title=retranscodificar'), array('update' => 'files_mms', 'url' => 'transcoders/retrans?id='.$transcoding->getId().'&mm='.$mm.'&preview=true&mod=' . $module, 'script' => 'true'))?>
          <?php if($transcoding->getStatusId() == TranscodingPeer::STATUS_ERROR) echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=delete title=delete'), array('update' => 'files_mms', 'url' => 'transcoders/deletefromfile?id='.$transcoding->getId().'&mm='.$mm.'&preview=true&mod=' . $module, 'script' => 'true'))?>
        </td>
        <td>
  ...
  <?php $t = count($files) ; for( $i=0; $i<$t; $i++): $file = $files[$i] //idea primer y ultimo apate ?>  
    <tr>
      ...
      <td><?php echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=borrar title=borrar'), array('update' => 'files_mms', 'url' => 'files/delete?id='.$file->getId().'&mm='.$mm.'&preview=true&mod=' . $module, 'script' => 'true', 'confirm' => '&iquest;Seguro?'))?></td>
      <?php if (sfConfig::get('app_videoserv_browser')) echo '<td>'.link_to_remote(image_tag('admin/mbuttons/auto_inline.gif', 'alt=autocompletar title=autocompletar'), array('update' => 'files_mms', 'url' => 'files/autocomplete?id='.$file->getId().'&mm='.$mm.'&preview=true&mod=' . $module, 'script' => 'true')).'</td>'?>
          ...
          <div class="list_options">
            <ul style="">
              <li><?php echo link_to_remote('Auto', 
					    array('update' => 'pic_mms', 'url' => 'files/pic?id='.$file->getId().'&mm='.$file->getMmId().'&preview=true&mod=' . $module, 'script' => 'true', 'loading' => '$("pic_mms_load").show()'))?></li>
              <li><?php echo link_to_remote('10%', 
					    array('update' => 'pic_mms', 'url' => 'files/pic?id='.$file->getId().'&mm='.$file->getMmId().'&preview=true&numframe=10%&mod=' . $module, 'script' => 'true', 'loading' => '$("pic_mms_load").show()'))?></li>
              <li><?php echo link_to_remote('25%', 
					    array('update' => 'pic_mms', 'url' => 'files/pic?id='.$file->getId().'&mm='.$file->getMmId().'&preview=true&numframe=25%&mod=' . $module, 'script' => 'true', 'loading' => '$("pic_mms_load").show()'))?></li>
              <li><?php echo link_to_remote('50%', 
					    array('update' => 'pic_mms', 'url' => 'files/pic?id='.$file->getId().'&mm='.$file->getMmId().'&preview=true&numframe=50%&mod=' . $module, 'script' => 'true', 'loading' => '$("pic_mms_load").show()'))?></li>
              <li><?php echo link_to_remote('75%', 
					    array('update' => 'pic_mms', 'url' => 'files/pic?id='.$file->getId().'&mm='.$file->getMmId().'&preview=true&numframe=75%&mod=' . $module, 'script' => 'true', 'loading' => '$("pic_mms_load").show()'))?></li>
              <li><?php echo link_to_remote('90%', 
					    array('update' => 'pic_mms', 'url' => 'files/pic?id='.$file->getId().'&mm='.$file->getMmId().'&preview=true&numframe=90%&mod=' . $module, 'script' => 'true', 'loading' => '$("pic_mms_load").show()'))?></li>
      ...
      <?php endif ?>
      <td><?php echo (( $i == 0) ? '&nbsp;' : (link_to_remote('&#8593;', array('update' => 'files_mms', 'url' => 'files/up?id='.$file->getId().'&mm='.$mm.'&preview=true&mod=' . $module, 'script' => 'true'))))   ?></td>
      <td><?php echo (( $i == $t-1)? '&nbsp;' : (link_to_remote('&#8595;', array('update' => 'files_mms', 'url' => 'files/down?id='.$file->getId().'&mm='.$mm.'&preview=true&mod=' . $module, 'script' => 'true')))) //dos espacios para misma anchura que flecha?></td>
      <td>
        ...
        (<?php echo basename($file->getFile()) ?>/<?php echo $file->getLanguage()->getName() ?>)
         ...
         <span style="font-size: 80%; font-style:italic;">
           <?php echo ($file->getPerfil()->getMaster())?'(Master)':''?>
           <?php echo ($file->getDisplay())?'':'(Oculto)'?>
           <?php echo ($file->getDownload())?'':'(Descarga denegada)'?>
         </span>
      </td>
    </tr>
  <?php endfor; ?>
