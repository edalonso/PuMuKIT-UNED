<h2>
 ...
 <?php echo strftime("%B", mktime(0, 0, 0, $m, 10)) ?>
</h2>
<table cellspacing="0" class="tv_admin_list">
            ...
            <span style="position:absolute; top:0px; right:0px"><?php echo $cal[$dweek][$d]?></span>
             ...
             <ul style="padding-left: 6px; list-style-type: none; ">
               <?php foreach($events as $e): $total_event++?>
                 <li>
                    <?php echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=borrar title=borrar'), array('update' => 'list_events', 'url' => 'events/delete?cal=cal&id='.$e->getId(), 'script' => 'true', 'confirm' => 'Seguro que desea borrar el evento de fecha ' . $e->getDate('d/m/Y H:i'). '?'))?>
                    <?php echo m_link_to($e->getTitle(), 'events/edit?cal=cal&id=' . $e->getId(), array('title' => 'Editar Novedad '.$e->getId()), array('width' => '800')) ?>
                 </li>
               <?php endforeach?>
             </ul>
          <?php endif?>
  ...
  <tfoot>
   ...
   &nbsp;<?php echo date("F", mktime(0, 0, 0, $m, 10)).'-'.$y?>&nbsp;
