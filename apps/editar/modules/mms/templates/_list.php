...
<table cellspacing="0" class="tv_admin_list" style="float:left;">
  ...
  <?php $t = count($mms) ; for( $i=0; $i<$t; $i++): $mm = $mms[$i]; $odd = fmod($i, 2) ?>
    <tr onmouseover="Element.addClassName(this,'tv_admin_row_over')" onmouseout="Element.removeClassName(this,'tv_admin_row_over')" class="tv_admin_row_<?php echo $odd ?><?php if($mm['id'] == $sf_user->getAttribute('id', null, 'tv_admin/mm')) echo ' tv_admin_row_this'?>" >
      ...
      <?php endif?>
      <td>
	 <?php echo ((($page == 1)&&( $i == 0)) ? '&nbsp;' : (link_to_remote('&#8593;', array('update' => 'list_mms', 'url' => 'mms/up?id='.$mm['id'], 'script' => 'true'), array('class' => 'updown', 'title' => 'Mover una posición hacia arriba'))).(link_to_remote('&#8657;', array('update' => 'list_mms', 'url' => 'mms/top?id='.$mm['id'], 'script' => 'true'), array('class' => 'updown', 'title' => 'Mover al inicio de la lista'))))   ?>
      </td>
      <td>
	 <?php echo ((($page == $total)&&( $i == $t-1))? '&nbsp;' : (link_to_remote('&#8595;', array('update' => 'list_mms', 'url' => 'mms/down?id='.$mm['id'], 'script' => 'true'), array('class' => 'updown', 'title' => 'Mover una posición hacia abajo'))).(link_to_remote('&#8659;', array('update' => 'list_mms', 'url' => 'mms/bottom?id='.$mm['id'], 'script' => 'true'), array('class' => 'updown', 'title' => 'Mover al final de la lista')))) ?>
      </td>
      <td>
...
<?php if (isset($reload_pub_channel)): ?>
  <?php echo javascript_tag("
    new Ajax.Updater('list_pub_" . $mm_sel . "', '" . url_for('mms/updatelistpub?id=' . $mm_sel) . "')
  "); ?>
<?php endif ?>
