...
<form target="iframeUpload"  method="post" enctype="multipart/form-data" action="<?php echo url_for('mmwizard/endfile')?>">
...
<div class="form-row">
  <?php echo label_for('file_type', 'Modo de ingesta:', 'class="required" ') ?>
  
  <div class="content">
    <input type="radio" name="file_type" value="file" checked="checked" onclick="$('input_trans_file').hide();$('input_trans_url').show();"/> Archivo Local  
    <input type="radio" name="file_type" value="url" onclick="$('input_trans_url').hide();$('input_trans_file').show();"/> Archivo de Servidor
  </div>
</div>
...
<div class="form-row" id="input_trans_file" style="display: none">
  ...
  <div class="content">
    <?php $value = input_tag('file', '' ,array ('size' => 80)); echo $value ? $value : '&nbsp;' ?> &nbsp;
