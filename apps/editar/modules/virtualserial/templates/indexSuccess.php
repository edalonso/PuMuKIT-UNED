...
window.create_div_in_table = function(cat, mm_id, idcat_to_add){
  if ( idcat_to_add == " . $cat_raiz_unesco->getId() . " ){
    ...
    div.insert(a1);
  } else if ( idcat_to_add == " . $cat_raiz_uned->getId() . " ) {
    var td = $('bs-docs-example-tematicas');
    var div = new Element('div', { 'id': 'cat-' + cat.id, 'style' : 'font-size: 10px; margin: 1px 5px 1px 1px', 'class': 'label label-success'}).update(cat.name);
  }
  //Add quit logica.
  td.insert(div);
};
    ...
    onSuccess: function(response){
        for (var i=0; i<response.responseJSON.added.length; i++) {
            ...
            if (c.mm_id == mmSelId && c.group.length!=0 && c.group[1]!=undefined) {
               ...
               if ( idcat_to_add == " . $cat_raiz_unesco->getId() . " || idcat_to_add == " .  $cat_raiz_uned->getId() . " ){ //UNESCO O TEMATICAS
