...
<?php foreach($serial->getMmsPublicAutonPub() as $v): $f= $v->getFileByPerfil(array(22, 34, 48)); if(is_null($f)) continue; ?>
  <item>
    <title><![CDATA[<?php echo (strlen($v->getTitle()) == 0?$serial->getTitle():$v->getTitle())?> ]]></title>
...
<?php foreach($serial->getMmsPublicAutonPub() as $v): $f= $v->getFileByPerfil(array(20, 21, 32, 33)); if(is_null($f)) continue; ?>
  <item>
    <title><![CDATA[<?php echo (strlen($v->getTitle()) == 0?$serial->getTitle():$v->getTitle())?>]]></title>
    <itunes:subtitle><![CDATA[<?php echo $v->getSubtitle()?>]]></itunes:subtitle>
    <itunesu:category itunesu:code="<?php echo $v->getCategoryItunes() ?>" />
