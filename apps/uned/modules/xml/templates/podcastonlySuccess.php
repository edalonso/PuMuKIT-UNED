<?php echo '<?xml version="1.0" encoding="UTF-8"?>' ?>
  <rss xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:itunesu="http://www.itunesu.com/feed" xml:lang="en" version="2.0">
    <channel>
      <title><?php echo str_replace('&', '&amp;', $serial->getTitle()) ?></title>
      <link><?php echo url_for($serial->getUrl(), true)?></link>
      <description><?php echo str_replace('&', '&amp;', $serial->getDescription()) ?></description>
      ...
      <itunes:summary><?php echo str_replace('&', '&amp;', $serial->getDescription())?></itunes:summary>
      <itunes:subtitle><?php echo $serial->getSubtitle() ?></itunes:subtitle>
...
<?php foreach($serial->getMmsPublicAutonPub() as $v): $f= $v->getFileByPerfil(array(22, 34, 48)); if(is_null($f)) continue; ?>
<?php if($f->getLanguage()->getCod() != strtoupper($culture)) continue;?>
  <item>
    <title><?php echo str_replace('&', '&amp;', (strlen($v->getTitle()) == 0?$serial->getTitle():$v->getTitle()))?></title>
    <itunes:subtitle><?php echo $v->getSubtitle()?></itunes:subtitle>
    <itunes:summary><![CDATA[ <?php echo $v->getDescription() ?> ]]></itunes:summary>
    <description><![CDATA[ <?php echo $v->getDescription() ?> ]]></description>
    ...
    <itunes:author>UVigo-TV</itunes:author>
    <itunes:keywords><?php echo str_replace('&', '&amp;', $v->getKeyword()) ?></itunes:keywords>
    <itunes:explicit>no</itunes:explicit>
