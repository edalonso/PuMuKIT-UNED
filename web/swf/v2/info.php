<?php

/**


DESCRIPCION INICIAL: 

Los reproductores “embedidos” con códigos de la vieja plataforma Flash también se mantendrán operativos. 
Para esto será necesario emplear y conservar el reproductor flash de la actual plataforma CanalUNED dentro del nuevo PuMuKIT.

Con este desarrollo los videos de UNED incrustados en páginas de terceros seguirán funcionando como hasta ahora pero servidos desde PuMuKIT. 
(Estos videos incrustados seguirán sin funcionar en dispositivos de tipo iPad igual que no lo hacen ahora)


--
DECESION FINAL:

Se mostrará una imagen indicando que se actualice el player, para ello (http://wiki.swftools.org/wiki/Main_Page)

png2swf -o CTVPlayer.swf image.png


--
Archivos SWF necesarios:


 - http://www.canaluned.com/swf/v2/CTVPlayer.swf
 - http://www.canaluned.com/swf/data/es/videos/embed/1/4/6/1/11641.xml
 - http://www.canaluned.com/locale/es_ES/player.properties
 - http://www.canaluned.com/swf/v2/plugins/playicon.swf
 - http://www.canaluned.com/swf/v2/plugins/logoPlugin.swf
 - http://www.canaluned.com/swf/v2/plugins/controlbar.swf
 - http://www.canaluned.com/swf/v2/plugins/subtitles.swf
 - http://www.canaluned.com/swf/v2/plugins/LiveSessions.swf
 - http://www.canaluned.com/swf/v2/plugins/AuthoringPlugin.swf
 - http://www.canaluned.com/swf/v2/plugins/statistics.swf
 - http://www.canaluned.com/swf/v2/plugins/SUPlugin.swf
 - http://www.canaluned.com/assets/mosca.png
 - http://www.canaluned.com/xml/contents/es/sessions/1/4/6/1/sessions_11641.xml
 - http://www.canaluned.com/assets/teleactos.jpg


