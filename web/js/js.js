...
var tabMenuClass = new Class.create();
tabMenuClass.prototype = {
    initialize : function(hash){
        var q = 'editEvent';
        if ((['#editEventHash', '#sessionEventHash', '#serialEventHash'].indexOf(hash)) != -1){
            q =hash.substring(1, hash.length - 4);
        }
	this.activo = q;
	Element.addClassName($(q), 'siSel');
...
window.change_select = function(elemento, selector)
{
        ...
        if (confirm('Pausar videos')) {
            new Ajax.Updater('list_transcoders', '/editar.php/transcoders/pause', {
                ...
                onSuccess:  selector.selectedIndex = 0,
		            parameters: 'ids='+ Object.toJSON($$('.transcoder_checkbox:checked').invoke('getAttribute', 'id'))}
			    );
        }
        break;
    case 'transc_play':
    ...
    case 'delete_sel':
		...
		evalScripts: true,
                onSuccess:  selector.selectedIndex = 0,
		parameters: 'ids='+Object.toJSON(seleccionados.invoke('getAttribute', 'id'))}
			    );
        }
        break;
    case 'delete_sel_virtual':
        seleccionados = $$('.' + elemento + '_checkbox:checked');
        if (seleccionados.length == 0) break;
        if (confirm('Seguro')) {
            new Ajax.Updater('list_'+elemento+'s', '/editar.php/virtualserial/delete', {
		asynchronous: true,
		evalScripts: true,
                onSuccess:  selector.selectedIndex = 0,
		parameters: 'ids='+Object.toJSON(seleccionados.invoke('getAttribute', 'id'))}
			    );
            new Ajax.Updater('jstree', '/editar.php/virtualserial/tree' , {
                asynchronous:true,
                evalScripts:true
            });
        }
    ...
    case 'hono_person_sel':
	    ...
	    evalScripts: true,
            onSuccess:  selector.selectedIndex = 0,
	    parameters: 'ids='+Object.toJSON($$('.'+elemento+'_checkbox:checked').invoke('getAttribute', 'id'))}
			);
        break;
      //separar honores sel
