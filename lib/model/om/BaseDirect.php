  ...
  protected $culture;
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->direct_type_id = $rs->getInt($startcol + 3);

			$this->resolution_hor = $rs->getInt($startcol + 4);

			$this->resolution_ver = $rs->getInt($startcol + 5);

			$this->calidades = $rs->getString($startcol + 6);

			$this->debug = $rs->getBoolean($startcol + 7);
			...
			return $startcol + 8; // 8 = DirectPeer::NUM_COLUMNS - DirectPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[2] => $this->getUrl(),
			$keys[3] => $this->getDirectTypeId(),
			$keys[4] => $this->getResolutionHor(),
			$keys[5] => $this->getResolutionVer(),
			$keys[6] => $this->getCalidades(),
			$keys[7] => $this->getDebug(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setDirectTypeId($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setResolutionHor($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setResolutionVer($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setCalidades($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setDebug($arr[$keys[7]]);
	}
	 ...
	 * If this collection has already been initialized with
	 * an identical criteria, it returns the collection.
	 * Otherwise if this Direct is new, it will return
	 * an empty collection; or if this Direct has previously
	 * been saved, it will retrieve related Events from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Direct.
	 */
	public function getEventsJoinSerial($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseEventPeer.php';
		if ($criteria === null) {
		...
		if ($this->collEvents === null) {
			if ($this->isNew()) {
				...
				$this->collEvents = EventPeer::doSelectJoinSerial($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * @throws     PropelException
	 */
	public function getEventsWithI18n($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseEventPeer.php';
		if ($criteria === null) {
		...
		if ($this->collEvents === null) {
			if ($this->isNew()) {
			   $this->collEvents = array();
				...
				$this->collEvents = EventPeer::doSelectWithI18n($criteria, $this->getCulture(), $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
		...
		if ($this->collSessions === null) {
			if ($this->isNew()) {
			   $this->collSessions = array();
				...
				$criteria->add(SessionPeer::DIRECT_ID, $this->getId());
	 ...
	 * actually need in Direct.
	 */
	public function getSessionsJoinEvent($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseSessionPeer.php';
		if ($criteria === null) {
		...
		if ($this->collSessions === null) {
			if ($this->isNew()) {
				...
				$this->collSessions = SessionPeer::doSelectJoinEvent($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
