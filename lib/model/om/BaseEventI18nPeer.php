...
abstract class BaseEventI18nPeer {
    ...
    foreach (sfMixer::getCallables('BaseEventI18nPeer:addDoSelectRS:addDoSelectRS') as $callable)
    {
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					...
					$temp_obj2->addEventI18n($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initEventI18ns();
				$obj2->addEventI18n($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
	...
	/**
	 * Method to DELETE all rows from the event_i18n table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
