...
abstract class BasePic extends BaseObject  implements Persistent {
		...
		if ($this->collPicEvents === null) {
			if ($this->isNew()) {
			   $this->collPicEvents = array();
				...
				$criteria->add(PicEventPeer::PIC_ID, $this->getId());
	 ...
	 * actually need in Pic.
	 */
	public function getPicEventsJoinEvent($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BasePicEventPeer.php';
		if ($criteria === null) {
		...
		if ($this->collPicEvents === null) {
			if ($this->isNew()) {
				...
				$this->collPicEvents = PicEventPeer::doSelectJoinEvent($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
