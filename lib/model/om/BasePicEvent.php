...
/**
 * Base class that represents a row from the 'pic_event' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePicEvent extends BaseObject  implements Persistent {
	...
	public function setPicId($v)
	{
		...
		if ($this->pic_id !== $v) {
			$this->pic_id = $v;
			$this->modifiedColumns[] = PicEventPeer::PIC_ID;
		}
	...
	public function setOtherId($v)
	{
		...
		if ($this->other_id !== $v) {
			$this->other_id = $v;
			$this->modifiedColumns[] = PicEventPeer::OTHER_ID;
		}
	...
	public function setRank($v)
	{
		...
		if ($this->rank !== $v || $v === 0) {
			$this->rank = $v;
			$this->modifiedColumns[] = PicEventPeer::RANK;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = PicEventPeer::NUM_COLUMNS - PicEventPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PicEvent object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BasePicEvent:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			PicEventPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BasePicEvent:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePicEvent:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePicEvent:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PicEventPeer::doInsert($this, $con);
					...
					$affectedRows += PicEventPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PicEventPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PicEventPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PicEventPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(PicEventPeer::DATABASE_NAME);

		if ($this->isColumnModified(PicEventPeer::PIC_ID)) $criteria->add(PicEventPeer::PIC_ID, $this->pic_id);
		if ($this->isColumnModified(PicEventPeer::OTHER_ID)) $criteria->add(PicEventPeer::OTHER_ID, $this->other_id);
		if ($this->isColumnModified(PicEventPeer::RANK)) $criteria->add(PicEventPeer::RANK, $this->rank);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PicEventPeer::PIC_ID, $this->pic_id);
		$criteria->add(PicEventPeer::OTHER_ID, $this->other_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PicEvent (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PicEvent Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
		if ($this->aEvent === null && ($this->other_id !== null)) {
			// include the related Peer class
			include_once 'lib/model/om/BaseEventPeer.php';

			$this->aEvent = EventPeer::retrieveByPK($this->other_id, $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = EventPeer::retrieveByPK($this->other_id, $con);
			   $obj->addEvents($this);
			 */
		}
		return $this->aEvent;
	...
	public function getEventWithI18n($con = null)
	{
		if ($this->aEvent === null && ($this->other_id !== null)) {
			...
			$this->aEvent = EventPeer::retrieveByPKWithI18n($this->other_id, $this->getCulture(), $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = EventPeer::retrieveByPKWithI18n($this->other_id, $this->getCulture(), $con);
			   $obj->addEvents($this);
			 */
		}
		return $this->aEvent;
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePicEvent:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePicEvent::%s', $method));
    }
