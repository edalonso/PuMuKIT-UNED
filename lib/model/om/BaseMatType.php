...
abstract class BaseMatType extends BaseObject  implements Persistent {
  ...
  protected $culture;
		...
		if ($this->collMaterialEvents === null) {
			if ($this->isNew()) {
			   $this->collMaterialEvents = array();
				...
				$criteria->add(MaterialEventPeer::MAT_TYPE_ID, $this->getId());
	 ...
	 * actually need in MatType.
	 */
	public function getMaterialEventsJoinEvent($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseMaterialEventPeer.php';
		if ($criteria === null) {
		...
		if ($this->collMaterialEvents === null) {
			if ($this->isNew()) {
				...
				$this->collMaterialEvents = MaterialEventPeer::doSelectJoinEvent($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
