...
abstract class BaseSerial extends BaseObject  implements Persistent {
  ...
  protected $culture;
		...
		if ($this->collEvents === null) {
			if ($this->isNew()) {
			   $this->collEvents = array();
				...
				$criteria->add(EventPeer::SERIAL_ID, $this->getId());
	 ...
	 * actually need in Serial.
	 */
	public function getEventsJoinDirect($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseEventPeer.php';
		if ($criteria === null) {
		...
		if ($this->collEvents === null) {
			if ($this->isNew()) {
				...
				$this->collEvents = EventPeer::doSelectJoinDirect($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
