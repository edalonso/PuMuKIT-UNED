...
abstract class BaseMm extends BaseObject  implements Persistent {
	...
	/**
	 * Collection to store aggregation of collUnedMediaOlds.
	 * @var        array
	 */
	protected $collUnedMediaOlds;

	/**
	 * The criteria used to select the current contents of collUnedMediaOlds.
	 * @var        Criteria
	 */
	protected $lastUnedMediaOldCriteria = null;
  ...
  protected $culture;
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collUnedMediaOlds !== null) {
				foreach($this->collUnedMediaOlds as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getUnedMediaOlds() as $relObj) {
				$copyObj->addUnedMediaOld($relObj->copy($deepCopy));
			}
	...
	/**
	 * Temporary storage of collUnedMediaOlds to save a possible db hit in
	 * the event objects are add to the collection, but the
	 * complete collection is never requested.
	 * @return     void
	 */
	public function initUnedMediaOlds()
	{
		if ($this->collUnedMediaOlds === null) {
			$this->collUnedMediaOlds = array();
		}
	}
	 ...
	 * an identical criteria, it returns the collection.
	 * Otherwise if this Mm has previously
	 * been saved, it will retrieve related UnedMediaOlds from storage.
	 * If this Mm is new, it will return
	 * an empty collection or the current collection, the criteria
	 * is ignored on a new object.
	 *
	 ...
	 * @throws     PropelException
	 */
	public function getUnedMediaOlds($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseUnedMediaOldPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
		...
		if ($this->collUnedMediaOlds === null) {
			if ($this->isNew()) {
			   $this->collUnedMediaOlds = array();
				...
				$criteria->add(UnedMediaOldPeer::MM_ID, $this->getId());

				UnedMediaOldPeer::addSelectColumns($criteria);
				$this->collUnedMediaOlds = UnedMediaOldPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				UnedMediaOldPeer::addSelectColumns($criteria);
				if (!isset($this->lastUnedMediaOldCriteria) || !$this->lastUnedMediaOldCriteria->equals($criteria)) {
					$this->collUnedMediaOlds = UnedMediaOldPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastUnedMediaOldCriteria = $criteria;
		return $this->collUnedMediaOlds;
	}

	/**
	 * Returns the number of related UnedMediaOlds.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      Connection $con
	 * @throws     PropelException
	 */
	public function countUnedMediaOlds($criteria = null, $distinct = false, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseUnedMediaOldPeer.php';
		if ($criteria === null) {
		...
		return UnedMediaOldPeer::doCount($criteria, $distinct, $con);
	}

	/**
	 * Method called to associate a UnedMediaOld object to this object
	 * through the UnedMediaOld foreign key attribute
	 *
	 * @param      UnedMediaOld $l UnedMediaOld
	 * @return     void
	 * @throws     PropelException
	 */
	public function addUnedMediaOld(UnedMediaOld $l)
	{
		$this->collUnedMediaOlds[] = $l;
		$l->setMm($this);
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collUnedMediaOlds) {
				foreach ((array) $this->collUnedMediaOlds as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
